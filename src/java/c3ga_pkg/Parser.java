...
package c3ga_pkg;
	...
	public class Parser {
		...
		private double[] coord = new double[32];
		...
		private final void sum(final BasisBlade bb) {
			...
			coord[idx] += bb.getScale() / (double)c3ga.BasisElementSignByIndex[idx];
		}
		...
		private final mv parse() throws ParseException {
			...
			while (true) {
				...
				while ((token == T_PLUS) || (token == T_MINUS)) { // accept all +- 
					cnt++;
					startIdx = endIdx+1;
					if (token == T_MINUS) bb.multiply(-1.0); // -
					token = getNextToken();
				}
				...
				{ 
					if (token == T_NUMBER) {
						{ // multiply scale with value of number
							try {
								bb.multiply((double)Double.parseDouble(substring(startIdx, endIdx)));
							} catch (Exception ex) {
